import { memo } from "react";
import { Handle, Position, type NodeProps } from "@xyflow/react";
import { Workflow } from "lucide-react";
import type { StreamFlowNodeData } from "../stream-builder";

export const ChildProcessorNode = memo(({ data, selected }: NodeProps) => {
  const d = data as StreamFlowNodeData;
  const componentName = d.component || d.componentId || "Processor";
  const hasCustomLabel = d.label && d.label !== componentName && d.label !== d.componentId;
  const configPreview = d.configYaml?.trim()
    ? d.configYaml.trim().split("\n")[0].slice(0, 30)
    : "";

  return (
    <div
      className={`relative rounded-md border bg-card text-card-foreground shadow-sm w-[150px] border-l-[3px] border-l-blue-400 cursor-grab active:cursor-grabbing ${
        selected ? "ring-2 ring-primary" : ""
      }`}
    >
      <Handle
        type="target"
        position={Position.Top}
        className="!w-0 !h-0 !border-0 !bg-transparent !min-w-0 !min-h-0"
        isConnectable={false}
      />

      <div className="px-2 py-1.5">
        <div className="flex items-center gap-1.5 mb-0.5">
          <Workflow className="h-3 w-3 text-blue-400" />
          <span className="text-[9px] uppercase tracking-wider text-muted-foreground font-medium">
            {componentName}
          </span>
        </div>
        {hasCustomLabel && (
          <p className="text-xs font-medium truncate">{d.label}</p>
        )}
        {configPreview ? (
          <p className="text-[9px] text-muted-foreground/70 truncate mt-0.5 font-mono">
            {configPreview}
          </p>
        ) : (
          <p className="text-xs text-muted-foreground truncate">not configured</p>
        )}
      </div>

      <Handle
        type="source"
        position={Position.Bottom}
        className="!w-0 !h-0 !border-0 !bg-transparent !min-w-0 !min-h-0"
        isConnectable={false}
      />
    </div>
  );
});

ChildProcessorNode.displayName = "ChildProcessorNode";
